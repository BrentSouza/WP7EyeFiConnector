...
{
    public class EyeFiRequestHandler : IModule
    {
        ...
        private string UploadPhoto(IRequest request)
        {
            ...
            XDocument doc = XDocument.Parse(request.Parameters.Get("SOAPENVELOPE").Value);
            EyeFiContentHandler contentHandler = new EyeFiContentHandler(doc);

            // Get the newly uploaded file into memory
            using (IsolatedStorageFile store = IsolatedStorageFile.GetUserStoreForApplication())
            {
                using (IsolatedStorageFileStream untrustedFile = new IsolatedStorageFileStream(request.Files["FILENAME"].TempFileName, FileMode.Open, store))
                {
                    ...
                    string unverifiedDigest = request.Parameters.Get("INTEGRITYDIGEST").Value;
                    ...
                    if (verifiedDigest == unverifiedDigest)
                    {
                        ...
                        using (IReader reader = tarball.ExtractAllEntries())
                        {
                            while (reader.MoveToNextEntry())
                            {
                                ...
                                {
                                    string imagePath = Path.Combine(App.ViewModel.DownloadLocation, reader.Entry.FilePath);
                                    imagePath = imagePath.Substring(0, imagePath.IndexOf('.') + 4);
